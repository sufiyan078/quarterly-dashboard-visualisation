...
export default function DataValidation() {
  ...
  const [parsedPage, setParsedPage] = useState<number>(1);
  const [selectedIgnoredRow, setSelectedIgnoredRow] = useState<IgnoredInventoryRow | null>(null);
  const [showAllSheetsLog, setShowAllSheetsLog] = useState<boolean>(false);

  const getDetailedReasonExplanation = (row: NeedsReviewInventoryRow) => {
    const codes = row.validationCodes || [];
    if (codes.includes("MISSING_BOTH_QUANTITIES")) {
      return "Both ERP stock and physical count quantities are blank. At least one quantity is required to compute physical-system variance.";
    }
    if (codes.includes("INVALID_QUANTITY")) {
      return "The quantity column contains non-numeric symbols or characters (e.g. letters, invalid symbols), preventing variance analysis.";
    }
    if (codes.includes("MISSING_ITEM_CODE")) {
      return "Omitted item identifier code. A valid item code is mandatory to track inventory transactions and map records.";
    }
    if (codes.includes("MISSING_DESCRIPTION")) {
      return "Item description is blank. While the item can still be parsed, it is flagged for description completeness.";
    }
    if (codes.includes("INVALID_VALUE")) {
      return "The financial value column has non-numeric formats (e.g. text or invalid currency markers), which blocks total value calculations.";
    }
    if (codes.includes("UNEXPECTED_BLANK")) {
      return "More than 50% of core standard fields are empty, signaling potential data alignment errors in this row.";
    }
    if (codes.includes("DUPLICATE_ITEM")) {
      return "This item code is duplicate and appears across multiple sheets. Review is advised to check for potential double-counting.";
    }
    return "This row contains formatting discrepancies or missing attributes that prevent correct parsing into active inventory items.";
  };
  ...
  const totalParsedPages = Math.ceil(parsedRows.length / rowsPerPage);
  const parsedPreviewRows = parsedRows.slice((parsedPage - 1) * rowsPerPage, parsedPage * rowsPerPage);
    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="space-y-6 max-w-[1600px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="rounded-xl border border-slate-800 bg-[#0c0e14]/50 p-4.5 flex flex-col justify-between h-full">
            ...
            <div className="grid grid-cols-2 gap-y-3 gap-x-2 pt-3 border-t border-slate-900/60 mt-3">
              <div>
          ...
          <div className="rounded-xl border border-slate-800 bg-[#0c0e14]/50 p-4.5 flex flex-col justify-between h-full">
            ...
            <div className="space-y-3 pt-3 border-t border-slate-900/60 mt-3">
              <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="rounded-xl border border-slate-800 bg-[#0c0e14]/50 p-4.5 flex flex-col justify-between h-full">
            <div className="space-y-3">
              ...
              <div>
                ...
                {dataProfile.valueProfile.financialValueStatus === "not_found" ? (
                  ...
                  <span className="font-mono font-black text-[19px] text-amber-400 block mt-0.5">
                    0.00 SAR (All zeroes)
                  </span>
                ) : (
                  <span className="font-mono font-black text-[19px] text-emerald-400 block mt-0.5">
            ...
            {/* Added detail metrics to fill empty space */}
            <div className="py-2.5 my-2 border-t border-b border-slate-900/40 grid grid-cols-2 gap-3 text-[10px]">
              <div>
                <span className="text-slate-500 block text-[9px] uppercase tracking-wider font-semibold">Average Item Value</span>
                <span className="font-mono font-bold text-slate-300 text-[11px] block mt-0.5">
                  {dataProfile.rowAccountability.importedRows > 0
                    ? `${(dataProfile.valueProfile.totalInventoryValue / dataProfile.rowAccountability.importedRows).toLocaleString(undefined, { maximumFractionDigits: 2 })} SAR`
                    : "0.00 SAR"
                  }
                </span>
              </div>
              <div>
                <span className="text-slate-500 block text-[9px] uppercase tracking-wider font-semibold">Valuation Coverage</span>
                <span className="font-mono font-bold text-slate-300 text-[11px] block mt-0.5">
                  {dataProfile.rowAccountability.importedRows > 0
                    ? `${(((dataProfile.rowAccountability.importedRows - dataProfile.valueProfile.zeroValueCount - dataProfile.valueProfile.blankOrInvalidValueCount) / dataProfile.rowAccountability.importedRows) * 100).toFixed(1)}%`
                    : "0.0%"
                  }
                </span>
              </div>
              <div className="col-span-2">
                <span className="text-slate-500 block text-[9px] uppercase tracking-wider font-semibold">Highest Value Asset</span>
                <span className="font-mono text-slate-350 text-[10px] block truncate mt-0.5" title={dataProfile.valueProfile.topHighestValueItems[0] ? `${dataProfile.valueProfile.topHighestValueItems[0].item} - ${dataProfile.valueProfile.topHighestValueItems[0].description}` : "N/A"}>
                  {dataProfile.valueProfile.topHighestValueItems[0] 
                    ? `${dataProfile.valueProfile.topHighestValueItems[0].item} (${dataProfile.valueProfile.topHighestValueItems[0].value.toLocaleString(undefined, { maximumFractionDigits: 0 })} SAR)` 
                    : "N/A"
                  }
                </span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-1 pt-3 text-[9px] border-t border-slate-900/60 mt-3">
              <div>
          ...
          <div className="rounded-xl border border-slate-800 bg-[#0c0e14]/50 p-4.5 flex flex-col justify-between h-full">
            <div className="space-y-3">
              ...
              {dataProfile.quantityProfile.hasQuantityData && (
                <div className="grid grid-cols-3 gap-2 text-[10px]">
                  ...
                  <div>
                    <span className="text-slate-500 block text-[9px]">Variation</span>
                    <span className={`font-mono font-bold text-xs ${
                      (dataProfile.quantityProfile.totalSystemOnHand - dataProfile.quantityProfile.totalPhysicalCount) > 0 
                        ? "text-amber-455" 
                        : (dataProfile.quantityProfile.totalSystemOnHand - dataProfile.quantityProfile.totalPhysicalCount) < 0 
                          ? "text-rose-455" 
                          : "text-emerald-455"
                    }`}>
                      {(dataProfile.quantityProfile.totalSystemOnHand - dataProfile.quantityProfile.totalPhysicalCount).toLocaleString()}
                    </span>
                  </div>
                </div>
              )}
            ...
            {/* Added detail metrics/placeholders to fill empty space */}
            {dataProfile.quantityProfile.hasQuantityData ? (
              <div className="py-2.5 my-2 border-t border-b border-slate-900/40 grid grid-cols-2 gap-3 text-[10px]">
                <div>
                  <span className="text-slate-500 block text-[9px] uppercase tracking-wider font-semibold">Stock Accuracy Rate</span>
                  <span className="font-mono font-bold text-slate-300 text-[11px] block mt-0.5">
                    {dataProfile.rowAccountability.importedRows > 0
                      ? `${(((dataProfile.rowAccountability.importedRows - dataProfile.quantityProfile.variationMismatchCount) / dataProfile.rowAccountability.importedRows) * 100).toFixed(1)}%`
                      : "100.0%"
                    }
                  </span>
                </div>
                <div>
                  <span className="text-slate-500 block text-[9px] uppercase tracking-wider font-semibold">Variance Ratio</span>
                  <span className="font-mono font-bold text-slate-300 text-[11px] block mt-0.5">
                    {dataProfile.quantityProfile.totalSystemOnHand > 0
                      ? `${((Math.abs(dataProfile.quantityProfile.totalSystemOnHand - dataProfile.quantityProfile.totalPhysicalCount) / dataProfile.quantityProfile.totalSystemOnHand) * 100).toFixed(2)}%`
                      : "0.00%"
                    }
                  </span>
                </div>
                <div className="col-span-2">
                  <span className="text-slate-500 block text-[9px] uppercase tracking-wider font-semibold">Net Discrepancy Status</span>
                  <span className={`font-mono text-[10px] block font-bold mt-0.5 ${
                    (dataProfile.quantityProfile.totalPhysicalCount - dataProfile.quantityProfile.totalSystemOnHand) > 0 
                      ? "text-amber-450" 
                      : (dataProfile.quantityProfile.totalPhysicalCount - dataProfile.quantityProfile.totalSystemOnHand) < 0 
                        ? "text-rose-450" 
                        : "text-emerald-455"
                    ...
                    {(dataProfile.quantityProfile.totalPhysicalCount - dataProfile.quantityProfile.totalSystemOnHand) > 0 
                      ? `Surplus Stock (+${(dataProfile.quantityProfile.totalPhysicalCount - dataProfile.quantityProfile.totalSystemOnHand).toLocaleString()} Qty)` 
                      : (dataProfile.quantityProfile.totalPhysicalCount - dataProfile.quantityProfile.totalSystemOnHand) < 0 
                        ? `Deficit Stock (${(dataProfile.quantityProfile.totalPhysicalCount - dataProfile.quantityProfile.totalSystemOnHand).toLocaleString()} Qty)`
                        : "Perfect Alignment (0 Qty Variation)"
                    }
                  </span>
                </div>
              </div>
            ) : (
              <div className="py-6 my-2 border-t border-b border-slate-900/40 flex flex-col items-center justify-center text-center space-y-1">
                <Info className="h-5 w-5 text-slate-650" />
                <span className="text-slate-500 font-semibold text-[10px]">No quantity columns detected.</span>
                <span className="text-slate-600 text-[9px] max-w-[180px]">Map 'System On Hand' & 'Physical Count' to enable variance tracking.</span>
              </div>
            )}
            ...
            {dataProfile.quantityProfile.hasQuantityData ? (
              ...
              <div className="flex items-center justify-center py-4 border-t border-slate-900/60 mt-3">
          ...
          {(() => {
            ...
            return (
              <div className="rounded-xl border border-slate-800 bg-[#0c0e14]/50 p-5 space-y-4 flex flex-col justify-between h-full">
                <div className="space-y-3.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      <Building2 className="h-4.5 w-4.5 text-indigo-400" />
                      <span className="text-[11px] font-bold text-slate-350 uppercase tracking-wider">Business Concentration</span>
                    </div>
                    <span className="text-[9px] bg-indigo-500/10 border border-indigo-500/20 rounded px-2 py-0.5 text-indigo-400 font-mono font-extrabold uppercase tracking-wider">
                      {hasValues ? "By Value" : "By Volume"}
                    </span>
                  </div>

                  <div className="grid grid-cols-2 gap-4 pb-3 border-b border-slate-900/60">
                    <div>
                      <span className="text-slate-500 block text-[10px] uppercase tracking-wider font-bold">Top Supplier</span>
                      <span className="text-[13px] font-extrabold text-white block truncate mt-1" title={topSupplierName}>
                        {topSupplierName}
                      </span>
                      <span className="text-indigo-400 font-mono text-[11px] block font-bold mt-0.5">
                        {topSupplierMetric}
                      </span>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[10px] uppercase tracking-wider font-bold">Top Org/Division</span>
                      <span className="text-[13px] font-extrabold text-white block truncate mt-1" title={topOrgName}>
                        {topOrgName}
                      </span>
                      <span className="text-indigo-400 font-mono text-[11px] block font-bold mt-0.5">
                        {topOrgMetric}
                  ...
                  <div className="space-y-2.5">
                    <span className="text-slate-500 block text-[10px] uppercase tracking-wider font-bold">
                      {hasValues ? "Highest Value Assets" : "Highest Quantity Assets"}
                    </span>
                    {topItems.length > 0 ? (
                      <div className="space-y-2">
                        {topItems.map((item, index) => (
                          <div key={index} className="flex justify-between items-center text-xs bg-slate-900/40 rounded-lg p-2.5 border border-slate-800/60">
                            <div className="truncate flex-1 pr-3">
                              <span className="font-mono text-indigo-400 font-bold mr-2 text-[11px]">{item.item}</span>
                              <span className="text-slate-300 text-[11px] truncate" title={item.description}>
                                {item.description || "No Description"}
                              </span>
                            </div>
                            <span className="font-mono font-bold text-slate-100 flex-shrink-0 text-xs">
                              {hasValues 
                      ...
                      <span className="text-xs text-slate-500 italic block">No items to analyze.</span>
                    )}
                  </div>
                </div>

                <div className="text-[10px] text-slate-500 leading-normal border-t border-slate-900/60 pt-3 flex items-start gap-2">
                  <Info className="h-4 w-4 text-indigo-400/80 flex-shrink-0 mt-0.5" />
                  <span>
          ...
          <div className="rounded-xl border border-slate-800 bg-[#0c0e15]/40 backdrop-blur-md overflow-hidden">
            ...
            <div className="border-b border-slate-800 bg-slate-950/30 px-6 py-2 flex items-center justify-between flex-wrap gap-2">
              ...
              <span className="text-xs text-slate-500 font-mono">
                {activeTab === "parsed" && `Showing ${parsedRows.length > 0 ? (parsedPage - 1) * 50 + 1 : 0} - ${Math.min(parsedPage * 50, parsedRows.length)} of ${parsedRows.length} rows`}
              ...
              <div className="overflow-x-auto">
                {parsedRows.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-20 text-slate-500">
                    ...
                    <span className="text-xs text-slate-655 mt-1">Check needs review list to troubleshoot.</span>
                  </div>
                ) : (
                  <>
                    ...
                    {totalParsedPages > 1 && (
                      <div className="flex items-center justify-between px-6 py-4 border-t border-slate-800 bg-slate-950/20">
                        <div className="text-xs text-slate-400">
                          Page <span className="font-semibold text-white">{parsedPage}</span> of <span className="font-semibold text-white">{totalParsedPages}</span>
                        </div>
                        <div className="flex gap-2">
                          <button
                            onClick={() => setParsedPage(prev => Math.max(1, prev - 1))}
                            disabled={parsedPage === 1}
                            className="px-3 py-1.5 rounded-lg border border-slate-800 text-xs font-medium text-slate-355 hover:bg-slate-800/40 hover:text-slate-100 disabled:opacity-40 disabled:hover:bg-transparent disabled:cursor-not-allowed transition-all flex items-center gap-1 cursor-pointer"
                          >
                    ...
                    <tbody className="divide-y divide-slate-850 text-slate-400">
                      {ignoredRows.map((row, idx) => {
                        ...
                        return (
                          <tr 
                            key={idx} 
                            onClick={() => setSelectedIgnoredRow(row)}
                            className="hover:bg-slate-900/40 hover:text-slate-200 cursor-pointer transition-colors"
                          >
                            <td className="p-3 text-center font-mono text-slate-550">{row.originalRowNumber}</td>
                            <td className="p-3 font-semibold text-slate-355">{row.sheetName}</td>
                            ...
                            <td className="p-3 font-mono text-[10px] text-slate-550 truncate max-w-[350px]" title={cellPreviews}>
                              {cellPreviews || "—"}
                            </td>
                          </tr>
                        );
                  ...
                  <table className="w-full text-left text-[11px] border-collapse min-w-[1200px]">
                    <thead>
                      <tr className="bg-slate-950/60 border-b border-slate-850 text-slate-400 uppercase tracking-wider font-bold">
                        ...
                        <th className="p-3 w-28">Sheet</th>
                        <th className="p-3 w-24">Dataset Type</th>
                        <th className="p-3 w-32">Status</th>
                        <th className="p-3 w-40">Validation Codes</th>
                        <th className="p-3">Detailed Review Reason & Recommendation</th>
                        <th className="p-3 w-48">Fields Found / Missing</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-850 text-slate-400">
                      {needsReviewRows.slice(0, 100).map((row, idx) => {
                        ...
                        return (
                          <tr key={idx} className="hover:bg-slate-900/10 border-l-2 border-rose-500/40">
                            <td className="p-3 text-center font-mono text-slate-550">{row.originalRowNumber}</td>
                            <td className="p-3 font-semibold text-slate-355 truncate max-w-[120px]" title={`${row.sourceFileName} → ${row.sheetName}`}>{row.sheetName}</td>
                            <td className="p-3">
                            ...
                            <td className="p-3">
                              <span className="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-extrabold uppercase tracking-wide bg-rose-500/10 text-rose-450 border border-rose-500/20">
                                {row.reason.replace(/_/g, " ")}
                              </span>
                            </td>
                            <td className="p-3">
                              <div className="flex flex-wrap gap-1">
                                {(row.validationCodes || []).map((code, ci) => (
                                  <span key={ci} className="text-[8px] bg-slate-900 border border-slate-800 rounded px-1 py-0.5 text-slate-450 font-mono">
                                    {code}
                                  </span>
                                ))}
                              </div>
                            </td>
                            <td className="p-3 text-slate-200 leading-normal font-medium max-w-sm">
                              {getDetailedReasonExplanation(row)}
                            </td>
                            <td className="p-3 text-[10px]">
                              {row.mappedFieldsFound && row.mappedFieldsFound.length > 0 && (
                                <div className="flex flex-wrap gap-0.5 mb-1">
                                  {row.mappedFieldsFound.map((f, fi) => (
                                    <span key={fi} className="text-[8px] bg-emerald-500/5 border border-emerald-500/15 rounded px-1 py-0.5 text-emerald-400">
                                      {f}
                                    </span>
                                  ))}
                                </div>
                              ...
                              {row.missingFields && row.missingFields.length > 0 && (
                                <div className="flex flex-wrap gap-0.5">
                                  {row.missingFields.map((f, fi) => (
                                    <span key={fi} className="text-[8px] bg-rose-500/5 border border-rose-500/15 rounded px-1 py-0.5 text-rose-400 font-bold">
                                      ✗ {f}
                                    </span>
                                  ))}
                                </div>
                  ...
                  return (
                    <div className="rounded-xl border border-slate-800/80 bg-slate-950/20 p-5 space-y-5">
                      ...
                      <div className="border-t border-slate-850 pt-5 space-y-4">
                        ...
                        {/* Unmapped / Failed Row Mappings Section */}
                        {(() => {
                          const logRows = showAllSheetsLog 
                            ? [
                                ...ignoredRows.map(r => ({
                                  sheetName: r.sheetName,
                                  rowNumber: r.originalRowNumber,
                                  itemCode: (r.rawRowData.item || r.rawRowData.Item || r.rawRowData["Item Code"] || r.rawRowData["item_code"] || r.rawRowData["Part Number"] || r.rawRowData["Material"] || r.rawRowData["Material Number"] || r.rawRowData["Item Number"] || "—") as string,
                                  type: "Ignored Row",
                                  reason: r.reason
                                })),
                                ...needsReviewRows.map(r => ({
                                  sheetName: r.sheetName,
                                  rowNumber: r.originalRowNumber,
                                  itemCode: ((r as any).item || r.rawRowData.item || r.rawRowData.Item || r.rawRowData["Item Code"] || r.rawRowData["item_code"] || r.rawRowData["Part Number"] || r.rawRowData["Material"] || r.rawRowData["Material Number"] || r.rawRowData["Item Number"] || "—") as string,
                                  type: "Needs Review",
                                  reason: getDetailedReasonExplanation(r)
                                }))
                              ].sort((a, b) => a.sheetName.localeCompare(b.sheetName) || a.rowNumber - b.rowNumber)
                            : [
                                ...ignoredRows.filter(r => r.sheetName === diag.sheetName && r.sourceFileName === diag.sourceFileName).map(r => ({
                                  sheetName: r.sheetName,
                                  rowNumber: r.originalRowNumber,
                                  itemCode: (r.rawRowData.item || r.rawRowData.Item || r.rawRowData["Item Code"] || r.rawRowData["item_code"] || r.rawRowData["Part Number"] || r.rawRowData["Material"] || r.rawRowData["Material Number"] || r.rawRowData["Item Number"] || "—") as string,
                                  type: "Ignored Row",
                                ...
                                ...needsReviewRows.filter(r => r.sheetName === diag.sheetName && r.sourceFileName === diag.sourceFileName).map(r => ({
                                  sheetName: r.sheetName,
                                  rowNumber: r.originalRowNumber,
                                  itemCode: ((r as any).item || r.rawRowData.item || r.rawRowData.Item || r.rawRowData["Item Code"] || r.rawRowData["item_code"] || r.rawRowData["Part Number"] || r.rawRowData["Material"] || r.rawRowData["Material Number"] || r.rawRowData["Item Number"] || "—") as string,
                                  type: "Needs Review",
                            ...
                            <div className="border-t border-slate-850 pt-5 space-y-3">
                              <div className="flex items-center justify-between border-b border-slate-850 pb-3 flex-wrap gap-2">
                                <div className="space-y-0.5">
                                  <h5 className="text-xs font-bold text-rose-455 uppercase tracking-wider flex items-center gap-1.5">
                                    <AlertTriangle className="h-4 w-4 text-rose-450" />
                                    Unmapped / Failed Row Mappings Log
                                  </h5>
                                  <p className="text-[11px] text-slate-500">
                                    {showAllSheetsLog 
                                      ? "Showing all unmapped or ignored rows across all scanned worksheets."
                                      : `Detailed list of rows from sheet "${diag.sheetName}" that failed validation or were ignored.`
                                    }
                                  </p>
                                </div>
                                <div className="flex bg-slate-900/60 p-0.5 rounded-lg border border-slate-800">
                                  <button
                                    onClick={() => setShowAllSheetsLog(false)}
                                  ...
                                  >
                                    Selected Sheet ({[...ignoredRows.filter(r => r.sheetName === diag.sheetName && r.sourceFileName === diag.sourceFileName), ...needsReviewRows.filter(r => r.sheetName === diag.sheetName && r.sourceFileName === diag.sourceFileName)].length})
                                  </button>
                                  <button
                                    onClick={() => setShowAllSheetsLog(true)}
                                    className={`px-2.5 py-1 text-[10px] font-bold rounded-md transition-all cursor-pointer ${
                                      showAllSheetsLog 
                                        ? "bg-indigo-500 text-white shadow-sm font-extrabold"
                                        : "text-slate-400 hover:text-slate-200"
                                  ...
                                  >
                                    All Sheets ({ignoredRows.length + needsReviewRows.length})
                                  </button>
                                </div>
                              </div>

                              {logRows.length === 0 ? (
                                <div className="p-4 rounded-lg bg-emerald-500/5 border border-emerald-500/15 text-center text-xs text-emerald-400 font-semibold">
                                  ✓ All rows {showAllSheetsLog ? "across all sheets" : "in this sheet"} were successfully mapped and imported.
                                </div>
                              ) : (
                                <div className="border border-slate-850 rounded-lg overflow-hidden max-h-72 overflow-y-auto">
                                  <table className="w-full text-left text-[11px] border-collapse">
                                    <thead>
                                      <tr className="bg-slate-900/60 border-b border-slate-850 text-slate-400 font-bold sticky top-0 backdrop-blur-md">
                                        <th className="p-2.5 w-16 text-center">Row #</th>
                                        <th className="p-2.5 w-44">Worksheet Name</th>
                                        <th className="p-2.5 w-40">Item Number</th>
                                        <th className="p-2.5 w-24 text-center">Type</th>
                                        <th className="p-2.5">Detailed Mapping/Validation Issue</th>
                                      </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-850 text-slate-350 bg-[#0c0e15]/20">
                                      {logRows.map((item, uIdx) => (
                                        <tr key={uIdx} className="hover:bg-slate-900/40">
                                          <td className="p-2.5 text-center font-mono text-slate-550">{item.rowNumber}</td>
                                          <td className="p-2.5 font-semibold text-slate-300 truncate max-w-[150px]" title={item.sheetName}>{item.sheetName}</td>
                                          <td className="p-2.5 font-mono font-bold text-slate-200">
                                            {item.itemCode && String(item.itemCode).trim() !== "" ? String(item.itemCode) : <span className="text-slate-655 italic">Missing Code</span>}
                                          </td>
                                          <td className="p-2.5 text-center">
                                            <span className={`inline-flex items-center rounded px-1.5 py-0.5 text-[8px] font-bold border ${
                                              item.type === "Needs Review" 
                                                ? "bg-rose-500/10 border-rose-500/20 text-rose-455" 
                                                : "bg-slate-855 border-slate-800 text-slate-400"
                                            }`}>
                                              {item.type}
                                            </span>
                                          </td>
                                          <td className="p-2.5 text-slate-300 leading-normal">{item.reason}</td>
                                        </tr>
                                      ))}
                                    </tbody>
                                  </table>
      ...
      {/* Ignored Row Detail Modal */}
      {selectedIgnoredRow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="relative w-full max-w-2xl rounded-2xl border border-slate-800 bg-[#0c0e15]/95 shadow-2xl backdrop-blur-xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-slate-850 px-6 py-4">
              <div>
                ...
                <span className="text-[10px] text-slate-500 font-mono mt-0.5 block">
                  File: {selectedIgnoredRow.sourceFileName} &bull; Sheet: {selectedIgnoredRow.sheetName} &bull; Row: #{selectedIgnoredRow.originalRowNumber}
                </span>
              </div>
              <button
                onClick={() => setSelectedIgnoredRow(null)}
                className="text-slate-400 hover:text-slate-200 hover:bg-slate-800/40 p-1.5 rounded-lg transition-all cursor-pointer"
              >
            ...
            <div className="p-6 space-y-4 overflow-y-auto text-xs">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-slate-500 block uppercase text-[10px] font-bold">Category</span>
                  <span className={`inline-flex items-center rounded px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wide border mt-1.5 ${
                    selectedIgnoredRow.category === "ignored_header_row" ? "bg-cyan-500/10 border-cyan-500/20 text-cyan-400" :
                    selectedIgnoredRow.category === "ignored_empty_row" ? "bg-slate-800 text-slate-400 border border-slate-700/50" :
                    selectedIgnoredRow.category === "ignored_total_row" ? "bg-amber-500/10 border-amber-500/20 text-amber-450" :
                    "bg-purple-500/10 border-purple-500/20 text-purple-400"
                  }`}>
                    {selectedIgnoredRow.category.replace("ignored_", "").replace("_row", "")}
                  </span>
                </div>
                <div>
                  <span className="text-slate-550 block uppercase text-[10px] font-bold">Row Number</span>
                  <span className="font-mono text-sm font-bold text-slate-200 block mt-1.5">
                    {selectedIgnoredRow.originalRowNumber}
                  </span>
                </div>
              </div>

              <div>
                <span className="text-slate-550 block uppercase text-[10px] font-bold">Why was this ignored?</span>
                <div className="mt-1.5 p-3 rounded-lg border border-rose-500/25 bg-rose-500/5 text-slate-350 leading-relaxed">
                  <p className="font-medium text-slate-200">{selectedIgnoredRow.reason}</p>
                  <p className="text-[10px] text-slate-500 mt-1">
                    This row was classified under <span className="font-mono text-slate-300 font-semibold">{selectedIgnoredRow.category}</span> and was omitted from final audit calculations.
                  </p>
                </div>
              </div>

              <div>
                <span className="text-slate-555 block uppercase text-[10px] font-bold mb-2">Raw Excel Cell Data</span>
                <div className="border border-slate-850 rounded-lg overflow-hidden">
                  <table className="w-full text-left text-[11px] border-collapse">
                    <thead>
                      <tr className="bg-slate-900/60 border-b border-slate-850 text-slate-400 font-bold">
                        <th className="p-2.5">Column Name</th>
                        <th className="p-2.5">Cell Value</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-850 text-slate-350 bg-[#0c0e15]/25">
                      {Object.entries(selectedIgnoredRow.rawRowData).map(([colName, colVal], colIdx) => (
                        <tr key={colIdx} className="hover:bg-slate-900/40">
                          <td className="p-2.5 font-semibold text-slate-400">{colName}</td>
                          <td className="p-2.5 font-mono text-slate-200 break-all">
                            {colVal !== null && colVal !== undefined && String(colVal).trim() !== "" ? String(colVal) : <span className="text-slate-650 italic">empty</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
